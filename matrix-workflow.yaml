  ...
  pre_chamber_deployment:
    ...
    - id:  step1
      ...
      run: |
        ...
        echo "depoy_env=$(jq -r '.tags.Environment' $env_file )" >> $GITHUB_ENV
        echo "cc_env_value=$( [ "$(jq -r '.tags.Environment' $env_file )" = "Production" ] && echo "prod" || echo "non-prod" )" >> $GITHUB_ENV
        echo "cc_cham=$(jq -r '.settings.cc_chamber' $env_file )" >> $GITHUB_ENV
        echo "cc_env=$(jq -r '.settings.cc_env' $env_file )" >> $GITHUB_ENV
        cc_ch=$(jq -r '.settings.cc_chamber' $env_file )
        echo "::set-output name=ccid::$(echo $deploy_type)"
        echo "::set-output name=cc_chamber_id::$(echo $cc_ch)"
